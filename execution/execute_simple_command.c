...
/*   Updated: 2025/06/25 13:47:54 by lduflot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "execution.h"

//Accede à une variable globale déclarée ailleur dans une lib, celle-ci contiens toutes les variables d'env.
extern char **environ;
...
int execute_simple_command(t_treenode *node)
{
	...
	pid_t pid = fork(); // crée processus enfant 
	if (pid == 0) //enfant
	{
		//on recherche la cmd dans les bin de path
		char	*cmd_path = NULL; //stok le chemin de la commande
